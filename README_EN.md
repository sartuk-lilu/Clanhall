...
Holding down LMB puts the character into combat stance - WASD stops moving you and starts attacking instead: overhead, left, right, low. Regular movement doesn't go away, it just moves to Shift - hold it down and WASD goes back to strafing and backing up, at a speed that depends on the weapon in your hands, not the character. Which attack plays next isn't decided by a fixed combo string but by a pair of directions, "was → now": the system looks at what the previous hit was and picks the matching transition from a tree. Series length belongs to the weapon too - an unfamiliar blade is available at its full series length from the very first swing, no leveling required. During the wind-up, key presses are simply ignored; during the open follow-up window, the last press wins. Mistime it and the series drops into recovery and has to start over, but nothing is actually lost: a basic attack costs nothing and cancels instantly the moment you let go of LMB. Active skills work the opposite way - they commit the instant you press the button, and once committed they can't be interrupted.
...
There are no invincibility frames and no dodge roll - the spacebar dodge just moves your body out of the hit zone physically: get clear in time and the swing simply doesn't reach you, don't and it does. Parrying is the only defense that costs a resource. Any contact, whether it's a clash or a hit that landed clean, shuts down the recipient's own attack zone for the rest of that step - there's no hyper armor, so nobody trades hits just by virtue of having already started swinging.
