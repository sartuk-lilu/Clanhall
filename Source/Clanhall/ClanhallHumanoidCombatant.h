...
//
// Класс бойца — один ассет: ComboData, ClassTag и слоты
...
protected:
	...
	/** Флаг bStepParried, TryParry(). Стороне-нейтрален (`Parrying.md`) —
	 *  работает одинаково у игрока и у AI. */
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "AbilitySystem", meta = (AllowPrivateAccess = "true"))
	TObjectPtr<UClanhallParryComponent> ParryComponent;

	/** Класс бойца одним ассетом — ComboData, ClassTag и
	...
	/** Не UPROPERTY: значение одинаково у всех китов — это
...
public:
	 ...
	 *  того же класса, у них разный ранг. Ранга 0 не существует (`economy_system.md`, «Ранги и длина серии»):
	...
	/** (`combat_system.md`, «Stagger — усталость»): есть ли у ПРОТИВНИКА этого бойца (см. FindPrototypeOpponent)
