...
// на конкретном AnimNotifyState_Hitbox (`Animation Setup.md`, «Диспетчер зон поражения»). Одновременно может быть открыто
...
// НЕСКОЛЬКО ЗОН = НЕСКОЛЬКО УРОНОВ, ВСЕГДА (`DataAsset and Fragments.md`, «Мультизонные удары»): развилки «две формы одного
...
//
// Клэш парирования (`ability_system.md`, «Контрнавык»): проверяется только у зон с bParryable == true.
...
// переходе «были зоны -> зон не осталось» (`Animation Setup.md`, «Диспетчер зон поражения»).
...
public:
	...
	/** (`combat_system.md`, «Сквозной принцип: контакт сбивает зону получателя»): по владельцу пришёл любой контакт (клэш или пропущенный
	 ...
	 *  открыться), Event.Hitbox.Closed шлётся здесь явно: у UGA_DirectionalAttackBase на
	 *  контактном пути терминатора по концу монтажа нет вовсе, только этот ивент — без него
	 *  способность не заканчивается никогда (`DataAsset and Fragments.md`, «Терминатор способности»). На
	 *  State.SkillCommitted это не влияет: его единственная точка снятия — UGA_PhysicalSkill::EndAbility. */
	...
	/** (`Parrying.md`, «Кредит клэша»): кратковременно замедляет текущий проигрываемый монтаж
...
private:
	...
	/** (`combat_system.md`, «Сквозной принцип: контакт сбивает зону получателя»): взведено — BeginHitbox() отказывает, до сброса на
