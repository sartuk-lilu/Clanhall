...
// (GA_PhysicalSkill). Канон: все они работают только в боевой стойке
// (`combat_system.md`, «Боевая стойка и переключение режимов»).
// Основной путь урона у обоих — резолв по контакту зоны (`Combat Stance and WASD Attacks.md`,
// `DataAsset and Fragments.md`, «Два режима резолва урона»). Поиск цели
...
protected:
	/** Мгновенный поиск цели сферой перед персонажем. Не основной путь урона —
	...
	/** (`combat_system.md`, «AP — Armor Points»): снимает AP цели, 50% снятого возвращается атакующему в AP,
	 *  переполнение (урон больше остатка AP) идёт прямиком в HP. Возвращает true, если урон
	 *  был нанесён — это и есть "confirmed hit", который гейтит метки/синергии/ManaGain
